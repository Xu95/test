...
(function() {
	var Files = {
		...
		isFileNameValid: function (name) {
			...
			{
				throw t('files', '"{name}" 是无效的文件名', {name: name});
			} else if (trimmedName.length === 0) {
				throw t('files', '文件名不能为空');
			}
			return true;
		},
		displayStorageWarnings: function() {
			...
			if (usedSpacePercent > 98) {
				if (owner !== oc_current_user) {
					OC.Notification.show(t('files', '{owner} 的存储空间已满, 无法再更新或同步文件',
						{ owner: ownerDisplayName }));
					return;
				}
				OC.Notification.show(t('files', '你的存储空间已满，无法再更新或同步文件'));
				return;
			}
			if (usedSpacePercent > 90) {
				if (owner !== oc_current_user) {
					OC.Notification.show(t('files', '{owner} 的存储空间已用 ({usedSpacePercent}%)',
						{ usedSpacePercent: usedSpacePercent,  owner: ownerDisplayName }));
					return;
				}
				OC.Notification.show(t('files', '你的存储空间已用 ({usedSpacePercent}%)',
